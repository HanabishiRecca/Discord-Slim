...
export const Permissions = {
    equals: (a: string | number | bigint, b: string | number | bigint) =>
        BigInt(a) == BigInt(b),

    combine: (permissions: (string | number | bigint)[]) => {
        let result = 0n;
    ...
    check: (source: string | number | bigint, permission: Permission) =>
        (BigInt(source) & BigInt(permission)) == BigInt(permission),

    has: (source: string | number | bigint, permission: Permission) =>
        Permissions.check(source, Flags.ADMINISTRATOR) || Permissions.check(source, permission),

    add: (source: string | number | bigint, permission: Permission) =>
        (BigInt(source) | BigInt(permission)).toString(),

    remove: (source: string | number | bigint, permission: Permission) =>
        (BigInt(source) & ~BigInt(permission)).toString(),
};
...
export const CdnImages = {
    ...
    TeamIcon: (team: Team, size?: number, ext?: 'png' | 'jpg' | 'webp') =>
        team.icon ?
